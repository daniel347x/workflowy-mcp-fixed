...
async def _resolve_uuid_path_and_respond(target_uuid: str | None, websocket, format_mode: str = "f3") -> None:
    ...
    # Reject virtual UUIDs (generated by GLIMPSE for display, not real Workflowy IDs)
    if target.startswith("virtual_"):
        await websocket.send(json.dumps({
            "action": "uuid_path_result",
            "success": False,
            "target_uuid": target_uuid,
            "error": "Virtual UUIDs are not supported (these are temporary display IDs from GLIMPSE, not real Workflowy node IDs)",
        }))
        return
